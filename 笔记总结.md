...
总结：函数中如果存在yield，那么函 数就是一个生成器函数，调用生成器函数会返回一个生成器。生成器只有被for循环时，生成器函数内部的函数才会执行，每次循环都会获取yield返回的值。 
...
私有  只有自己能访问到。派生类也访问不了（只有内部能访问，外部访问不了。）
